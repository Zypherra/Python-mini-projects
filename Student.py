...
def search_user():
    ...
    try:
            ...
            if result:
                ...
                user_info.configure(font=("Arial", 14))
...
def open_registration():
    ...
    reg_window.configure(bg="#64adf7")
    #set window to full screen height and width
    reg_window.geometry(f"{screen_width}x{screen_height}")

    tk.Label(reg_window, text="Registration Form", font=("Times New Roman", 20,"bold"),background="#64adf7").grid(padx=20,pady=20)
    ...
    save_btn.grid(row=9,column=0,padx=20,pady=50)
    ...
    reset_btn.grid(row=9,column=1,padx=20,pady=50)
...
def open_crud():
    ...
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
    crud_window.configure(bg="#64adf7")
